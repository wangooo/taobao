...
<script>
  ...
  export default {
    ...
    methods:{
     getshopkeeper(){
       axios
       .get("/shopkeeper/getshopkeeper").then(res=>{
    ...
    clearshopkeeper(shopkeeper,index,rows){
       var con;
        ...
        if(con==true)
        {
          ...
          rows.splice(index,1)
           axios
           .get("/shopkeeper/clearshopkeeper",{params:{name:shopkeeper}}).then(res => {
                    this.shopkeeperList = res.data.result.list
                    console.log(res);
                });
    ...
    computed:{
      searchData(){
            if(this.input2){
                return this.shopkeeperList.filter((value)=>{  //过滤数组元素
                    return value.name.includes(this.input2)||value.city.includes(this.input2)||value.adress.includes(this.input2); //如果包含字符返回true
                });
            }
            else{
